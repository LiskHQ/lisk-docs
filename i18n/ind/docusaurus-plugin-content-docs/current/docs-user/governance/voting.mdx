...
1. Kunjungi [Lisk DAO](https://www.tally.xyz/gov/lisk) dan hubungkan dompet Anda ke Tally.
...
Sangat penting bagi anggota Lisk DAO untuk dapat berinteraksi dengan DAO kapan saja, bahkan ketika Tally, antarmuka standar untuk Lisk DAO, sedang tidak aktif.
Yang terpenting, pemungutan suara dan delegasi harus selalu bisa dilakukan dengan berinteraksi langsung dengan kontrak pintar.
...
async function main() {
    ...
    const logs = await provider.getLogs({
        fromBlock: 568778, // blok saat kontrak Governor dideploy
        toBlock: 'latest',
        address: "0x58a61b1807a7bDA541855DaAEAEe89b1DDA48568", // L2Governor Proxy
        topics:['0x7d84a6263ae0d98d3329bd7b46bb4e8d6f98cd35a7adb45c274c8b7fd5ebd5e0'] // tanda tangan acara `ProposalCreated`
    });
    console.log(logs);
}
...
Untuk memastikan proposal yang Anda cari, klik tab `Logs` dan cari acara `ProposalCreated`. Jika nilai-nilai, terutama untuk `description`, `voteStart`, dan `voteEnd`, cocok dengan proposal yang ingin Anda beri suara, maka itu adalah proposal yang benar. Untuk mendapatkan ID proposal, salin nilai dari kolom `proposalId`.
<img src={proposalId}/>
...
6. Klik tombol `Write`.
  <img src={voteSubmit}/>
7. Konfirmasi transaksi di dompet Anda.

Untuk memverifikasi bahwa suara Anda telah dikirim dengan benar, Anda dapat memeriksa acara `VoteCast` dari transaksi Anda.

[^1]: Pada saat penulisan, terdapat [masalah](https://github.com/blockscout/frontend/issues/2059) dengan menghubungkan MetaMask ke Blockscout di Firefox;
