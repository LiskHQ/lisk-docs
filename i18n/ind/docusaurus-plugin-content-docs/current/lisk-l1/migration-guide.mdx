...
#### Konfigurasi
Konfigurasi spesifik modul, yang sebelumnya berada di `config.json` pada Lisk L1, sekarang menjadi bagian dari kontrak pintar itu sendiri dan didefinisikan sebagai variabel state.
...
Untuk mengatur pemilik kontrak, kita menambahkan variabel state baru `owner`, dan sebuah konstruktor yang mengatur variabel `owner` ke alamat akun yang meluncurkan kontrak.
...
<Tabs>
  ...
  <TabItem value="liskl2" label="Lisk L2" default>
  Untuk getter sederhana, cukup tambahkan modifier visibilitas `public` pada variabel state (lihat [storage](#storage)).
...
Migrasikan endpoint modul dengan mengimplementasikan [fungsi view](https://solidity-by-example.org/view-and-pure-functions/) yang sesuai dalam kontrak seperti yang ditunjukkan di bawah ini.
...
<Tabs>
  ...
  <TabItem value="liskl2" label="Lisk L2" default>
  Untuk getter sederhana, cukup tambahkan [visibility modifier](https://solidity-by-example.org/visibility/) `public` pada variabel state (lihat [storage](#storage)).
...
Dengan menguji kontrak pintar, Anda dapat memverifikasi bahwa kontrak pintar berfungsi seperti yang diharapkan dan bebas dari bug, sebelum meluncurkannya ke Lisk.
...
Sekarang Anda dapat meluncurkan kontrak pintar ke Lisk.
Untuk contoh ini, kami akan menggunakan jaringan Lisk Sepolia untuk meluncurkan kontrak Hello.
...
Jika peluncuran berhasil, keluaran seharusnya terlihat seperti ini:
...
Setelah kontrak pintar diluncurkan, Anda dapat berinteraksi dengannya dengan memanggil fungsi publiknya.
