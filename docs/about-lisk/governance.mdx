...
:::info
Every LSK token holder who participates in the DAO is rewared with [staking rewards](#staking-rewards), to incentivize participation in the Lisk governance.
:::
...
:::note
The DAO fund may be increased further in the future by additional 100,000,000 LSK over the following 7 years, if the DAO members approve the corresponding proposals.
:::
...
But it is possible to [boost the voting power](#boosting-the-voting-power) by up to 200% where the boost is proportional to remaining locking duration.
...
### Delegation
The Governor framework requires to **delegate the voting power** in order to use it.
...
Delegation happens on [Tally](#tally).
...
### Proposals
Proposals are created on [Tally](#tally).
...
Funding proposals are **binding**, which means that no-one, not even the Onchain Foundation, can prevent the transfer, if the proposal passed.
...
Rewards are taken from the balance of the `Staking` constract and calculated on a daily basis, based on the amount locked and the remaining locking duration.
...
1. the unclaimed LSK tokens (recall they would be moved to Lisk DAO treasury after the 2-years claim window)
