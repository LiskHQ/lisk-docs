...
To verify the Hello message, we implement a custom modifiers in the contract.
...
Then you can conveniently check the length of Hello messages in the `validLength` modifier like this:
...
<Tabs>
  ...
  <TabItem value="liskl2" label="Lisk L2" default>
  {/* Are these functions required, or is it enough to declare the state variable public? */}  
    ```solidity title="hello_liskl2/src/Hello.sol"
...
Once the smart contract is [deployed](#smart-contract-deployment) to Lisk, you can [interact](#interacting-with-the-smart-contract) with it by calling its public functions.

Finally, you can migrate the [plugins and UI](#migration-of-plugins-and-ui) of the Lisk L1 Hello app to be compatible with the new API, to complete the migration process of your Lisk application.
...
Add the `--verify` flag to the `forge create` command to directly verify the smart contract on the Lisk network.
...
From here, you can migrate the plugins and UI of the Lisk L1 Hello app to be compatible with the new API, to complete the migration process of your Lisk application.
