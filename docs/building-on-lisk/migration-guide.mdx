...
- `add2Blacklist()` to configure the blacklist of words that are not allowed in the Hello message.
...
function add2Blacklist(string calldata _newWord) public onlyOwner {
    blacklist.push(_newWord);
} 
...
contract HelloTest is Test {
    ...
    function test_CreateHello() public {
        ...
        assertEq(hello.message(alice),"Hello World");
...
We will use the Lisk Sepolia network to deploy our the Hello contract for this example.

### Interacting with the smart contract

### Migration of plugins and UI
