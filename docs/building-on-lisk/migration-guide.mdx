...
Please investigate for cutom solutions to migrate logic residing in the lifecycle hooks.
For example, you could implement something like lazy evaluation to keep last evaluated block height in the state and compute reward only when it is requested.
