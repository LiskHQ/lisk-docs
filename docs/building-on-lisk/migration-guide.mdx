...
The `validMessage()` modifier that we defined above in the [Verification migration](/docs/building-on-lisk/migration-guide#state-transition-logic) section is used to check if the message is valid before the `createHello()` function is executed.
