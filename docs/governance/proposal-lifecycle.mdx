...
- **Quorum**: For a proposal to pass, the “yes” and “abstain” votes must sum up to at least  **24,000,000 vpLSK**, i.e., the value that corresponds to 8,000,000 LSK locked for 2 years where the countdown is paused.
- **Majority**: There must be strictly **more “yes” than “no”** votes.


### What happens when a proposal passed

- Funding proposals: one must first queue the proposal in Tally and then execute the proposal.
Anyone can do this, but each step requires a transaction.
- General proposals: Foundation will evaluate if they implement it or not
what happens when a proposal did not passed: nothing
