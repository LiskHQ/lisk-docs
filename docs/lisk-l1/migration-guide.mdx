...
The `validMessage()` modifier that we defined above in the [Verification migration](#state-transition-logic) section is used to check if the message is valid before the `createHello()` function is executed.
